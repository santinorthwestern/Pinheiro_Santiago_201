...
print(paste("The largest win spread is", max_spread, ", when the Bears scored", bears_score, "points and the Opponent scored", opponent_score,"points."))
...
```{r}
add_games <- matrix(c(37, 9, 17, 17), ncol = 2, byrow = FALSE)

game_score <- rbind(game_score, add_games)
colnames(add_games) <- c("bears", "opponent")
print(game_score)
```
...
```{r}
first_win <- match(TRUE, game_score[, "win"])
losses_beforewin <- first_win - 1

print(losses_beforewin)
```
...
area_of_rectangle <- function(length = 48, width = 48){
  area_sqinch <- length * width
  area_sqft <- area_sqinch / 144
  
  return(c(area_sqinch, area_sqft))
}
result <- area_of_rectangle(length = 102, width =60)
cat(sprintf("The area in square inches is %d.\n", result[1]))
cat(sprintf("The area in square feet is %.1f.", result[2]))
```
...
longest_word <- function(words) { 
  ...
  longest_w <- words[word_length == max_length]
  return(longest_w)
}

longest_fruit <- longest_word(fruits)
print(longest_fruit)
...
```{r}
is_in_vect <- function(value, vec){
  return(value %in% vec)
}


result <- is_in_vect(value, check)
...
x_mod <- convert_negative_to_NA(x)
...
```{r}
even_odd_convert <- function(mat){
  ifelse(mat %% 2 == 0, 1, 0)
}
x_mod2<- even_odd_convert(x)
print(x_mod2)
```
